...
if candle_time_key not in st.session_state:
    st.session_state[candle_time_key] = None

if alert_time_key not in st.session_state:
    st.session_state[alert_time_key] = 0.0
...
if has_open_trade:
    ...
    if triggered_close:
        ...
        st.session_state.journal.at[open_trade_idx, "ExitPrice"] = round(exit_p, precision_digits)
...
should_trigger = (
    ...
    and (signal != st.session_state[signal_key] or current_candle_time != st.session_state[candle_time_key])
    and (now - st.session_state[alert_time_key] > cooldown_sec)
)
...
if should_trigger:
    new_log = pd.DataFrame([{
        ...
        "Symbol": symbol,
        "Signal": signal,
        "Entry": round(entry, precision_digits),
        "SL": round(sl, precision_digits),
        "TP": round(tp, precision_digits),
        "ExitPrice": None,
        "PnL (%)": None,
        "Result": "OPEN"
    }])
    ...
    st.session_state[candle_time_key] = current_candle_time
    st.session_state[alert_time_key] = now

# -------------------- DISPLAY CURRENT METRICS --------------------
c1, c2, c3, c4 = st.columns(4)
c1.metric(f"{symbol} Price", f"${price:,.{precision_digits}f}")
...
if has_open_trade:
    ...
    pnl_label = f"+${live_pnl_cash:,.{precision_digits}f} ({live_pnl_pct:+.2f}%)" if live_pnl_cash >= 0 else f"-${abs(live_pnl_cash):,.{precision_digits}f} ({live_pnl_pct:.2f}%)"
    lc2.metric("Floating Live PnL", pnl_label, delta=f"{live_pnl_pct:.2f}%", delta_color="normal" if live_pnl_cash >= 0 else "inverse")
    
    lc3.metric("Stop Loss Level", f"${trade_row['SL']}", delta=f"Distance: ${abs(price - float(trade_row['SL'])):,.{precision_digits}f}", delta_color="inverse")
    lc4.metric("Take Profit Level", f"${trade_row['TP']}", delta=f"Distance: ${abs(float(trade_row['TP']) - price):,.{precision_digits}f}", delta_color="normal")
